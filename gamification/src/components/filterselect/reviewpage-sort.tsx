...
const ReviewsPageSortSelect: FC = () => {
    ...
    const onClickSort = (sort: string) => {
        if (pageParam === null) {
            if (sort !== 'false') {
                ...
                artilce_store.getAllArticleFunc(1, pageLimitElement, sort, 'review')
            }
            if (sort === 'false') {
                setSearchParams({})
                artilce_store.getAllArticleFunc(1, pageLimitElement, null, 'review')
            }
        }
...
export default observer(ReviewsPageSortSelect);