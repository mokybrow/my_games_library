...
async def create_article_router(
    ...
    # if not cover:
    #     base64_string = None
    result = await create_article(title=model.title, text=model.text, snippet=model.snippet,tags=model.tags, user_id=user.id, db=db)
    if not result:
...
async def edit_article_router(
    model: articles_model.ArticleEditModel,
    db: AsyncSession = Depends(get_async_session),
    user: User = Depends(active_user_with_permission),
):
    if not user.reporter:
    ...
    result = await edit_article(title=model.title, text=model.text, snippet=model.snippet,tags=model.tags, article_id= model.article_id, db=db)
    if not result:
        error = error_model.ErrorResponseModel(details='List with this name already exist')
        return JSONResponse(
            content=error.model_dump(),
...
async def get_all_article_router(
    ...
    result = await get_all_article(offset=page, limit=limit, sort=sort,tag=tag, user_id=user_id, db=db)
    if not result:
