...
async def get_all_games_router(
    ...
    result = await get_all_games(db=db, page=page, limit=limit,sort=sort, decade=decade, genre=genre)
    if not result:
...
async def get_games_count_router(
    limit: int = None,
    genre: Optional[str] = None,
    db: AsyncSession = Depends(get_async_session),
):
    result = await get_count_games(db=db, genre=genre)
